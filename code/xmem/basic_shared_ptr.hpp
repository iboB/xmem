...
public:
    ...
    template <typename U>
    basic_shared_ptr(const basic_shared_ptr<U, CBT>& other)
        : m_cb(other.m_cb)
        , m_ptr(other.m_ptr)
    {
    ...
    template <typename U>
    basic_shared_ptr& operator=(const basic_shared_ptr<U, CBT>& other) {
    ...
    template <typename U>
    basic_shared_ptr(basic_shared_ptr<U, CBT>&& other)
        : m_cb(other.m_cb)
        , m_ptr(other.m_ptr)
    {
        other.m_cb = nullptr;
    ...
    template <typename U>
    basic_shared_ptr& operator=(basic_shared_ptr<U, CBT>&& other) {
        if (m_cb) m_cb->dec_strong_ref();
        m_cb = other.m_cb;
        m_ptr = other.m_ptr;
        other.m_cb = nullptr;
    ...
    template <typename U>
    basic_shared_ptr(U* p) : basic_shared_ptr(unique_ptr<U>(p)) {}
    ...
    T* operator->() const noexcept { return m_ptr; }
    element_type& operator[](size_t i) const noexcept {return m_ptr[i]; }
    ...
    const void* owner() const noexcept { return m_cb; }

    template <typename U, typename UCBT>
    bool owner_before(const basic_shared_ptr<U, UCBT>& other) const noexcept {
